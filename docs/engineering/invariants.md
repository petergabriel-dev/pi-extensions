...
- Progress uses keyed `subagents-progress` widget updates throttled to 250ms; each visible run identifies `cmux` or `headless` transport and its runtime log; terminal states remove entries and clear widget when no runs remain (`agent/extensions/subagents/progress.ts`).
- Every run owns one Pi-runtime diagnostics log under `<agentDir>/subagents/<parent-session>/`, with directory mode `0700`, file mode `0600`, a 1 MiB persistent cap, an 8 KiB recent-output cap, exact IPC-token redaction, and no full environment/auth capture. Child stdout/stderr and failed cmux screen output are bounded; failures expose transport, log path, error, and tail once, while cmux surfaces still auto-close (`agent/extensions/subagents/diagnostics.ts`, `agent/extensions/subagents/launch.ts`, `agent/extensions/subagents/cmux.ts`).
