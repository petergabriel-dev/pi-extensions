...
- Background reconciliation passes `shouldContinue` from `triggerBackgroundReconciliation` in `agent/extensions/persistent-memory/index.ts`; if `lifecycleGeneration` changes between chunks, the loop stops and remaining candidates stay staged.
...
- `reconcile_attempts` increments only for candidates that were sent to a model and left unapplied after validation rejection. Budget-skipped candidates and candidates in model-error/timeout chunks are re-staged with attempts unchanged.
