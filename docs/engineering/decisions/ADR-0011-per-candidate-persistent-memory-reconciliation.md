...
- Isolate failures to the affected candidate or adjudication batch: failed/invalid candidates remain staged with `reconcile_attempts` backoff and are dead-lettered past the configured cap.
...
- Good: Reconciliation has clearer per-candidate conservation and retry semantics.
