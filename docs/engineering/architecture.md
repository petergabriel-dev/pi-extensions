...
`capture` accepts primary `sessionId` plus deprecated `claudeSessionId`; if both are present, `sessionId` wins. `capture` and `save_plan` use the Pi event bus rather than imported extension module state. `claude-bridge` emits `discussion-notes:add`; the live `discussion-notes` extension updates its own notes array, appends the session snapshot, and redraws the Notes widget. It does not stage memory candidates. `claude-bridge` emits `workflow-modes:save-plan`; the live `workflow-modes` extension writes its durable plan file, updates live state, appends `workflow-plan` as audit, and returns `workflow-modes:save-plan-result`. Recall gets saved-plan state only through live `workflow-modes:get`; unavailable live state is an error, never a bridge cache or direct plan-file read.
...
`agent/extensions/workflow-modes/plan-store.ts` owns saved plans under `~/.pi/agent/plans/<encoded-repo-root>/<sanitized-branch>.md`. It keys repositories by `git rev-parse --show-toplevel` (or cwd outside Git), atomically writes a frontmatter record (`planId`, `savedAt`) before live state changes, and uses last-write-wins. `workflow-modes/index.ts` reads this file on session restore; legacy `workflow-plan` entries are adopted once only when no file exists. Session entries remain best-effort audit, not durable truth.
