...
This repository publishes as `@lopezpetergabriel/pi-extensions@0.7.0`, one Pi package containing eleven extensions, four skills, and two package-owned bundled agent definitions. It is source, not a separate Pi home.
...
| 11 | `agent/extensions/ask-user-question/index.ts` | `ask_user_question` tool | Owns model-initiated text, single-select, and multi-select questions, module-local FIFO UI serialization, per-question Skip, cancel-all Escape handling, and the threshold-defer/resume flow. |
...
Five extensions have no outgoing cross-extension edge—neither a sibling import nor a `pi.events` call—in their complete runtime source: `ask-user-question` (`agent/extensions/ask-user-question/index.ts:1-778`, `agent/extensions/ask-user-question/queue.ts:1-40`, `agent/extensions/ask-user-question/defer.ts:1-54`), `ccc-search` (`agent/extensions/ccc-search/index.ts:1-183`), `filechanges` (`agent/extensions/filechanges/index.ts:1-586`), `notify` (`agent/extensions/notify.ts:1-55`), and `personal-memory` (`agent/extensions/personal-memory/index.ts:1-181`, `agent/extensions/personal-memory/curation.ts:1-137`, `agent/extensions/personal-memory/store.ts:1-246`). Browser uses the namespaced `browser:request`/`browser:result` event channel; incoming use of `personal-memory/store.ts` by `claude-bridge` remains visible in the static map above (`agent/extensions/claude-bridge/index.ts:8`). The new ask-user-question defer code uses only local imports and host lifecycle APIs, so this row remains accurate.

### ask_user_question context defer

After an answered TUI result, `index.ts:726-739` reads `ctx.getContextUsage()` once and records bounded question/answer text when `defer.ts:22-53` finds `tokens > contextWindow - 16,384`. `deferredThisRun` and `deferAbortRequested` keep one pending batch to one abort and one transcript marker.

`tool_execution_end` (`index.ts:660-671`) waits for `ask-user-question/queue.ts:8-40` to drain, appends the TUI-only `ask-user-question:context-defer` entry, then aborts. `agent_settled` (`index.ts:673-679`) takes the pending batch and sends `buildResumeMessage()` as a fresh user message without `deliverAs`; Pi's non-streaming prompt preflight checks compaction at `@earendil-works/pi-coding-agent/dist/core/agent-session.js:864-868`. `pi.appendEntry()` keeps the marker out of LLM context. A disposable AgentSession harness observed compaction, fresh resume, one batch marker/abort, preserved mixed-tool results, and no unpaired `tool_use`.
...
Children launch interactively with `pi --no-extensions -e agent/extensions/subagents/child.ts` and no `--print`; the child extension registers only loadout-approved browser/nested tools plus `ask_question`. `subagent_agents:` frontmatter controls nested definitions; `MAX_SUBAGENT_DEPTH` is two. Any toolset containing a mutating tool requires Build at spawn time. Concurrent children use one configurable default semaphore with capacity three; normalized overlapping ownership paths are rejected by `ownership.ts`.

`cmux.ts` is the required production transport: it creates an unfocused terminal surface, renames its tab, waits for a shell prompt, and sends the quoted child command. Binary, socket, auth, and surface failures close partial surfaces and reject with classified errors; `spawnProcess` remains only an explicit test injection seam. Each run captures child stdout/stderr in a Pi-owned `0600` log under `<agentDir>/subagents/<parent-session>/`, bounded to 1 MiB, with an in-memory recent-output tail bounded to 8 KiB and exact IPC-token redaction. Failed cmux runs best-effort capture bounded `read-screen` output before auto-close. The child caches final text from extension-level `agent_end`, then steers once on `agent_settled`, after retries, compaction, and queued continuation settle. Start, progress, list, and failure follow-up expose cmux transport and log path; `progress.ts` renders `subagents-progress` above the editor with 250ms throttling.
