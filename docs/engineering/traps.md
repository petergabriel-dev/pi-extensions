...
- **Model fallback can hide timeout root causes.** Extraction and adjudication resolve separate model roles and fall back to `ctx.model` when the pinned/env model is missing or lacks auth. Check resolved-model logs before assuming intended provider/model.
