...
export function isBashAllowedInMode(command: string, mode: "discuss" | "plan"): boolean {
	const normalized = command.trim().replace(/\s+/g, " ");
	const allowed = mode === "discuss" ? DISCUSS_BASH_ALLOW.test(normalized) : PLAN_BASH_ALLOW.test(normalized);
