...
Worker delegation:
- Use the worker-orchestration skill's A+B model for contract-first delegation. When \`subagent\` is available and the confirmed task is substantial (multi-file or multi-step changes), start exactly one worker per task inside the Section-4 loop with \`agent: "worker"\`. Pass task text as \`task\` and derive \`fileOwnership\` from task scope or target files. Use \`subagents_list\` to monitor; use \`subagent_message\` for live follow-up. Parent retains task selection, Verification Gate, commit, and confirmation; worker handles isolated implementation.
