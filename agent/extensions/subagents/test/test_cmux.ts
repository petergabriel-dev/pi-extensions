...
const surface = await transport.launch({ cwd: "/tmp/project", title: "worker", command: "pi", args: ["--print", "do work"], env: { PI_SUBAGENT_OWNER: "worker-one" } });
assert.ok(surface);
...
assert.equal(await fallback.launch({ cwd: "/tmp", title: "worker", command: "pi", args: [], env: {} }), undefined);
...
assert.equal(await notReady.launch({ cwd: "/tmp", title: "worker", command: "pi", args: [], env: {} }), undefined);
