...
async function testGenerationGuardStopsBetweenCandidates() {
  ...
  try {
    ...
    // Second candidate remains staged
    assert.strictEqual(stagingCount(mem), 1, "Staging preserved for second candidate");
    const staged = readStagedLessons(mem);
    assert.strictEqual(staged.length, 1);
    assert.strictEqual(staged[0].summary, "Lesson Beta", "Second candidate still staged");
    ...
    console.log("  ✓ committed 1 lesson,", "1 staged remaining");
  } finally {
    try { db.close(); } catch {}
    fs.rmSync(root, { recursive: true, force: true });
  }
...
async function testGenerationGuardStopsBeforeAdjudicationBatch() {
  ...
  try {
    ...
    // Collision candidate remains staged
    ...
    console.log("  ✓ staging preserved, sqlite unchanged");
  } finally {
    try { db.close(); } catch {}
    fs.rmSync(root, { recursive: true, force: true });
  }
